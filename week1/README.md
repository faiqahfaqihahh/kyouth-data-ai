...
Place all `.mhtml` source files inside `data/0_source/` before running the pipeline. All other directories (`data/1_bronze/`, `data/2_silver/`, `data/3_gold/`) are created automatically.
...
This command now runs from a fresh Gold database every time, so `data/3_gold/jobs.db` is recreated before loading.
...
python main.py report    # Print source/silver/gold counts and identify count differences
python main.py clean     # Remove the existing Gold database
```
...
This project is designed to be reproducible from a clean machine running Windows (tested with Python 3.10+).

- Create a virtual environment and install runtime deps (from project root):
...
- Run the pipeline after activating the venv:
...
- The `pyproject.toml` includes a minimal `[build-system]` table so `pip` can build/install the project if needed.
- Developer tools can be installed from the optional `dev` extras listed in `pyproject.toml`.
