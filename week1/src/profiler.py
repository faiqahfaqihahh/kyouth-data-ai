...
def run_data_profile(db_path):
    ...
    # Data quality tagging only (NO deletion)
    cursor.execute("""
    ...
    print(
        f"❓ Missing Values -> "
        f"job_title: {missing_titles}, "
        f"company: {missing_companies}, "
        f"description: {missing_descriptions}"
    )
    ...
    print(
        f"   ↳ source_id: {shortest_desc[0]} | "
        f"job_title: {shortest_desc[1]}"
    )

    print(f"🚨 Longest Description: {longest_desc[2]} chars")
    print(
        f"   ↳ source_id: {longest_desc[0]} | "
        f"job_title: {longest_desc[1]}"
    )